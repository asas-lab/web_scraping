...
# Set the path for the json data file
json_data_file_path = f'{current_path}\almasryalyoum.json'

# Set the path for the file to store failed links
failed_links_file = f'{current_path}\failed_links.json'
...
if os.path.exists(failed_links_file):
    df_failed = pd.read_json(failed_links_file)
else:
...
# Set the save rate for the json data file
save_rate = 5
...
for page_num in range(start_num_page, end_num_page+1):
    try:
        ...
        # Save the data to the json file at the specified save rate
        ...
        df_failed.to_json(failed_links_file, orient='records', lines=True, force_ascii=False)
        continue
